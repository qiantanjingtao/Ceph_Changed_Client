...
bool cal_distance(Inode* in) {
  struct Point {
    long double atime, btime, ctime, mtime;
    long int size;
  };
  Point su;
  su.atime = 439900; su.btime = 439880; su.ctime = 439880; su.mtime = 439880; su.size = 151816;

  long double proc = square(in->atime.tv.tv_sec/3600 - su.atime) + square(in->btime.tv.tv_sec/3600 - su.btime)
                     + square(in->ctime.tv.tv_sec/3600 - su.ctime) + square(in->mtime.tv.tv_sec/3600 - su.mtime)
                     + square(in->size-su.size);
  if (proc < 100000.0) {
       return true;
  }
...
Client::Client(Messenger *m, MonClient *mc, Objecter *objecter_, ceph::unordered_map <string, list<utime_t>> dentry_list)
  : Dispatcher(m->cct),
    m_command_hook(this),
    ...
    meta_lock("Client::meta_lock"), //changed code
    deleg_timeout(0),
    dcount(dentry_list)
{
  _reset_faked_inos();
...
Dentry *Client::insert_dentry_inode(Dir *dir, const string& dname, LeaseStat *dlease, 
				    ...
				    const UserPerm& request_perms, Dentry *old_dentry)
{
  Dentry *dn = NULL;
  ...
  if (dn && dn->inode) {
    if (dn->inode->vino() == in->vino()) {
      touch_dn(dn);
      dcount.add_single_dentry(dn);

      //changed
      // didi record(in, dn);
      LOG_ERROR("go, %s", (dn->name + ": " + std::to_string(in->atime) + " " + std::to_string(in->btime) + " " + std::to_string(in->ctime) + " " + std::to_string(in->mtime) + " " + std::to_string(dcount.get_list_size(dn)) + "\n").c_str());
      /* in 2

      if ((cal_distance(in) || dcount.get_list_size(dn) > 3 ) && !dcount.get_prefetch_flag(dn)) {
          meta_lock.Lock();
          dcount.set_prefetch_flag(dn);
          meta_lock.Unlock();

          ofstream outfile;
          outfile.open("log.txt", ios::app);
          outfile << "insert_dentry_inode function" << cal_distance(in) << " " << dcount.get_list_size(dn) << " " << dcount.get_prefetch_flag(dn) << "\n";
          outfile.close();

          check_prefetch(dn, request_perms);
      }
      */
...
void Client::insert_readdir_results(MetaRequest *request, MetaSession *session, Inode *diri) {
  ...
  if (!p.end()) {
	...
	if (olddn->inode != in) {
      ...
      if ((cal_distance(in) || dcount.get_list_size(dn) > 3 ) && !dcount.get_prefetch_flag(dn)) {
          dcount.set_prefetch_flag(dn);
          check_prefetch(dn, request->perms);
      }
      */
  ...
  if (reply->head.is_dentry) {
    ...
    if (in) {
      ...
      insert_dentry_inode(dir, dname, &dlease, in, request->sent_stamp, session,
                          request->perms, (op == CEPH_MDS_OP_RENAME) ? request->old_dentry() : NULL);
    } else {
      Dentry *dn = NULL;
    ...
    if (in) {
      Dir *dir = diri->open_dir();
      insert_dentry_inode(dir, dname, &dlease, in, request->sent_stamp, session, request->perms);
    } else {
      if (diri->dir && diri->dir->dentries.count(dname)) {
...
void Client::record(Inode *in, Dentry *dn) {
  if (in && dn) {
    // changed code
      
     string res = dn->name + ": " + std::to_string(in->atime) + " " + std::to_string(in->btime) + " " +
                   std::to_string(in->ctime) + " " + std::to_string(in->mtime) + " " + std::to_string(dcount.get_list_size(dn)) + "\n";
     LOG_ERROR("go, %s", res.c_str());
   }
}
		    ...
		    const UserPerm& perms)
{
 ...
 hit_dn:
  ...
  dcount.add_single_dentry(dn);
  // changed 
  // didi record((&(*(*target))), dn);
  if (dn->name != "no_dismount.txt" && dn->name != "vdb_control.file") 
  LOG_ERROR("go, %s", (dn->name + ": " + std::to_string((&(*(*target)))->atime) + " " + std::to_string((&(*(*target)))->btime) + " " + std::to_string((&(*(*target)))->ctime) + " " + std::to_string((&(*(*target)))->mtime) + " " + std::to_string(dcount.get_list_size(dn)) + "\n").c_str());
  /*
  if ((cal_distance(&(*(*target))) || dcount.get_list_size(dn) > 3 ) && !dcount.get_prefetch_flag(dn) && dn->name!="no_dismount.txt" && dn->name!="vdb_control.file") {
      meta_lock.Lock();
      dcount.set_prefetch_flag(dn);
      meta_lock.Unlock();
      ...
      outfile << "_lookup function " << dn->name << cal_distance(&(*(*target))) << " " << dcount.get_list_size(dn) << " "<< dcount.get_prefetch_flag(dn) << "\n";
      outfile.close();


      check_prefetch(dn, perms);
  }
 */
 done:
  if (r < 0)
...
// changed
/*
void Client::check_prefetch(Dentry* dn, const UserPerm& perms) {
  if (dcount.get_list_size(dn) != 0) {
        ofstream outfile;
        outfile.open("log.txt", ios::app);
     if (dn->inode && dn->inode->is_dir() && (ceph_clock_now().tv.tv_sec-dcount.last_call.tv.tv_sec>1))  {    // 当前是个目录, 检查前后两次的调用的时间差，避免短时间频繁调用
        dir_result_t *dirp = new dir_result_t(&(*(dn->inode)), perms);
        dirp->offset = 2;
        _readdir_get_frag(dirp);
        outfile << dcount.last_call.tv.tv_sec << "\n";
        dcount.last_call = ceph_clock_now();

        outfile << dcount.last_call.tv.tv_sec << "\n";
        outfile << dn->name << " is a dir\n";
     } else if(dn->dir && (ceph_clock_now().tv.tv_sec-dcount.last_call.tv.tv_sec>1)) {  // 当前是个文件
        dir_result_t *dirp = new dir_result_t(dn->dir->parent_inode , perms);
        dirp->offset = 2;
        _readdir_get_frag(dirp);
        outfile << dcount.last_call.tv.tv_sec << "\n";
        dcount.last_call = ceph_clock_now();
...
/*
  ofstream outfile;
  outfile.open("log.txt", ios::app);
  outfile << "return num is: " << res << " inode is: " << dirp->inode->ino << "\n";
  outfile.close();
*/
  if (res == -EAGAIN) {
...
/* in 2
    if ((cal_distance(in) || dcount.get_list_size(*dn) > 3 ) && !dcount.get_prefetch_flag(*dn)) {
       ...
       ofstream outfile;
       outfile.open("log.txt", ios::app);
       outfile << "ll_readlink function " << cal_distance(in) << " " << dcount.get_list_size(*dn) << " " << dcount.get_prefetch_flag(*dn) << "\n";
       outfile.close();
...
StandaloneClient::StandaloneClient(Messenger *m, MonClient *mc, ceph::unordered_map <string, list<utime_t>> dentry_list)
    : Client(m, mc, new Objecter(m->cct, m, mc, NULL, 0, 0), dentry_list)
{
