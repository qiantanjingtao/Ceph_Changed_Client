...
bool cal_distance(Inode* in) {
  double proc = fabs(in->atime.tv.tv_sec/3600 - hot.atime) + fabs(in->btime.tv.tv_sec/3600 - hot.btime)
                     + fabs(in->ctime.tv.tv_sec/3600 - hot.ctime) + fabs(in->mtime.tv.tv_sec/3600 - hot.mtime);
  if (proc < 10000.0) {
       return true;
  }
  ...
  if (dn && dn->inode) {
    if (dn->inode->vino() == in->vino()) {
      ...
      // didi record(in, dn);
       ...
       LOG_ERROR("debug, %s %s", "insert_dentry_inode", (dn->name).c_str()); 
      if (dn->name != "no_dismount.txt" && dn->name != "vdb_control.file") {
      if ((cal_distance(in) || dcount.get_list_size(dn) >= 8 ) && !dcount.get_prefetch_flag(dn)) {
          meta_lock.Lock();
...
void Client::insert_readdir_results(MetaRequest *request, MetaSession *session, Inode *diri) {
  ...
  if (!p.end()) {
	...
	if (olddn->inode != in) {
      ...
      //changed
        LOG_ERROR("debug, %s %s", "insert_readdir_results", (dn->name).c_str());
        if (dn->name != "no_dismount.txt" && dn->name != "vdb_control.file") {
         if ((cal_distance(in) || dcount.get_list_size(dn) >= 8 ) && !dcount.get_prefetch_flag(dn)) {
             meta_lock.Lock();
             dcount.set_prefetch_flag(dn);
		    ...
		    const UserPerm& perms)
{
 ...
 hit_dn:
  ...
  LOG_ERROR("debug, %s %s", "_look_up", (dn->name).c_str());

  if(dn->name!="no_dismount.txt" && dn->name!="vdb_control.file") {
  
    if ((cal_distance(&(*(*target))) || dcount.get_list_size(dn) >= 8 ) && !dcount.get_prefetch_flag(dn) && dn->name!="no_dismount.txt" && dn->name!="vdb_control.file") {
        meta_lock.Lock();
        dcount.set_prefetch_flag(dn);
        meta_lock.Unlock();
...
void Client::check_prefetch(Dentry* dn, const UserPerm& perms) {
  if (dcount.get_list_size(dn) != 0 && dn->name != "no_dismount.txt" && dn->name != "vdb_control.file") {
     LOG_ERROR("DEBUG, %s", (dn->name).c_str());
...
int Client::ll_readlink(Inode *in, char *buf, size_t buflen, const UserPerm& perms)
{
  ...
  while (dn != in->dn_set.end()) {
    ...
    // changed
    // didi record(in, *dn);
   LOG_ERROR("debug, %s %s", "ll_readlink", ((*dn)->name).c_str()); 
    if ((*dn)->name != "no_dismount.txt" && (*dn)->name != "vdb_control.file") {
      if ((cal_distance(in) || dcount.get_list_size(*dn) >= 8 ) && !dcount.get_prefetch_flag(*dn)) {
         meta_lock.Lock();
         dcount.set_prefetch_flag(*dn);
